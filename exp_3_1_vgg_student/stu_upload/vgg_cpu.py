...
class VGG19(object):  
    ...
    def build_model(self):
        # TODOï¼šå®šä¹‰VGG19 çš„ç½'ç»œç»"æž„
    ...
    def load_model(self):
        ...
        for layer_name in self.layers:
            ...
            print(f"{layer_name}: in={getattr(layer, 'channel_in', 'N/A')}, out={getattr(layer, 'channel_out', 'N/A')}")
        ...
        for idx in range(43):
            ...
            if 'conv' in layer_name:
                ...
                # 尝试不同的权重转置 - 这可能是关键问题
                # 原始: matconvnet [height, width, in_channel, out_channel] -> [in_channel, height, width, out_channel]
                weight = np.transpose(weight,[2,0,1,3])
                
                # 如果上面不行，尝试注释掉上一行，使用下面这行：
                # weight = np.transpose(weight,[3,2,0,1])  # [out_channel, in_channel, height, width]
    ...
    def load_image(self, image_dir):
        ...
        img = img.resize((224, 224))  # è°ƒæ•´å¤§å°ä¸º224x224
